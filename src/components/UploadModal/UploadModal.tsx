...
import { BULK_CREATE_STUDENTS } from "@mutations/students/BulkStudentUpload";
import { useMutation } from "@apollo/client";
// import { toast } from "react-toastify";
import type { BulkCreateStudentsResponse } from "@utils/types";
...
const UploadModal: React.FC<UploadModalProps> = ({ onClose, refetch }) => {
  const { t } = useTranslation();

  const [uploadFile] =
    useMutation<BulkCreateStudentsResponse>(BULK_CREATE_STUDENTS);
  const [fileName, setFileName] = useState<string>("");
  const [result, setResult] = useState<
    BulkCreateStudentsResponse["bulkCreateStudents"] | null
  >(null);
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      if (response.data) {
        setResult(response.data.bulkCreateStudents);
        ...
        <div className={styles.formContent}>
          <div className={styles.step}>
            ...
            <div className={styles.formGroup}>
              <label>{t("uploadModal.Upload Students via CSV")}</label>
              {fileLoading ? (
